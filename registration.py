...
firebase = firebase.FirebaseApplication(
    "https://project-c188-default-rtdb.firebaseio.com", None)
registration_window = Tk()
registration_window.geometry("400x400")
registration_window.config(bg="#a8caff")
...
def login():
    ...
    hexd = encoded.hexdigest()
    getp = firebase.get("/", username)
    if getp != None:
        if getp == hexd:
            messagebox.showinfo("Done", "Logged in")
        else:
            messagebox.showerror("Error", "Please check your password")


def register():
    global login_password_entry, login_username_entry
    ...
    Md5 = hashlib.md5(new)
    hexd = Md5.hexdigest()
    putdata = firebase.put("/", username, hexd)
    print(putdata)
    messagebox.showinfo("Done", "Data is added")
...
def login_window():
    ...
    login_window.config(bg="#a8caff")
    log_heading_label = Label(
        login_window, text="Log In", font='arial 18 bold', bg="#a8caff")
    log_heading_label.place(relx=0.5, rely=0.2, anchor=CENTER)

    login_username_label = Label(
        login_window, text="Username : ", font='arial 13', bg="#a8caff")
    ...
    login_password_label = Label(
        login_window, text="Password : ", font='arial 13', bg="#a8caff")
